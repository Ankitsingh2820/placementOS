...
function ScoutCard({ job, score, reason, action, onCoach }) {
  ...
  const pct = score >= 70 ? 'bg-emerald-500' : score >= 40 ? 'bg-amber-500' : 'bg-slate-300'
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-5 hover:shadow-md transition-all">
      <div className="flex justify-between items-start gap-2 mb-2">
        <div>
          <h3 className="font-semibold text-slate-900 text-sm">{displayJob.title || job?.title}</h3>
          <p className="text-slate-500 text-xs">{displayJob.company || job?.company}</p>
        </div>
      ...
      <p className="text-xs text-slate-600 mb-2 leading-relaxed"><span className="font-semibold text-primary">Why:</span> {reason}</p>
      {action && <p className="text-xs text-indigo-700 bg-indigo-50 rounded-lg px-3 py-2 mb-3"><span className="font-semibold">Action:</span> {action}</p>}
...
export function ScoutPage() {
  ...
  const [mode, setMode] = useState('search') // search | digest
  ...
  async function handleSearch() {
    ...
    setResults([])
    try {
      const data = await scoutJobs(query)
      setResults(data)
      setMode('search')
    } catch { }
  ...
  async function handleDigest() {
    ...
    setStatus('loading-digest')
    setDigest([])
    try {
      const data = await generateDigest(resumeText)
      setDigest(data)
      setMode('digest')
    } catch { }
    finally { setStatus('idle') }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <div className="mb-6">
        <h2 className="text-xl font-bold text-slate-900">Job Scout</h2>
        <p className="text-slate-500 text-sm mt-0.5">AI-powered search — describe what you want or get a personalized daily digest</p>
      </div>

      {/* Search bar */}
      <div className="bg-white border border-slate-200 rounded-xl p-5 mb-5">
        <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">What are you looking for?</label>
        <div className="flex gap-2">
          <input
            value={query}
      ...
      <div className="text-center mb-6">
        <button onClick={handleDigest} disabled={status === 'loading-digest'}
          className="inline-flex items-center gap-2 bg-indigo-500 hover:bg-indigo-600 disabled:opacity-60 text-white font-semibold px-8 py-3 rounded-xl text-sm transition-colors">
          <Sparkles size={16} />
          {status === 'loading-digest' ? 'Scanning…' : 'Generate My Top 3 Matches'}
        </button>
        <p className="text-xs text-slate-400 mt-2">Uses your resume from Interview Prep to find the best fits</p>
      </div>

      {/* Results */}
      {mode === 'search' && results.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">{results.length} matches found</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {results.map((r, i) => (
              <ScoutCard key={i} job={r.job} score={r.score} reason={r.reason} onCoach={handleCoach} />
            ))}
          </div>
        </div>
      )}
      ...
      {mode === 'digest' && digest.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Your top {digest.length} matches today</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {digest.map((r, i) => (
              <ScoutCard key={i} job={r} score={r.score} reason={r.reason} action={r.action} onCoach={handleCoach} />
            ))}
          </div>
        </div>
      )}
